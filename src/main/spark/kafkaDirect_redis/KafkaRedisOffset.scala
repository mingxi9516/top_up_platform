...
object KafkaRedisOffset {
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName(Constans.SET_APP_NAME).setMaster(Constans.SET_MASTER)
      // 设置没秒钟每个分区拉取kafka的速率
    ...
    // 配置基本参数
    // 组名
    val groupId = Constans.GROUP_ID
    // topic
    val topic = Constans.TOPIC
    // 指定Kafka的broker地址（SparkStreaming程序消费过程中，需要和Kafka的分区对应）
    val brokerList = ConfigManage.getProperties("brokerList")
    // 编写Kafka的配置参数
    ...
    stream.foreachRDD({
      rdd=>
        ...
        //调用方法，对每条json数据进行解析
        ParseJson.parseJsonData(value,ssc)
        // 将偏移量进行更新
